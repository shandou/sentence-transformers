...
source_embedding_file = "{}_{}_{}.emb".format(
    ...
    pca_dimensions if use_pca else model.get_sentence_embedding_dimension(),
)
...
if use_pca:
    ...
    with open(source_file, encoding="utf8") as fSource, open(
        target_file, encoding="utf8"
    ) as fTarget:
...
x2y_sim, x2y_ind = kNN(
    x, y, knn_neighbors, use_ann_search, ann_num_clusters, ann_num_cluster_probe
)
x2y_mean = x2y_sim.mean(axis=1)

y2x_sim, y2x_ind = kNN(
    y, x, knn_neighbors, use_ann_search, ann_num_clusters, ann_num_cluster_probe
)
...
for idx in range(len(bitext_list)):
    ...
    if labels[id1][id2] or labels[id2][id1]:
        ...
        if f1 > best_f1:
            ...
            best_recall = recall
            threshold = (
                bitext_list[idx][0] + bitext_list[min(idx + 1, len(bitext_list) - 1)][0]
            ) / 2
