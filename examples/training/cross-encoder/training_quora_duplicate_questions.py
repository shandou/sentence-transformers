...
train_samples = []
with open(
    os.path.join(dataset_path, "classification", "train_pairs.tsv"),
    "r",
    encoding="utf8",
) as fIn:
    ...
    for row in reader:
        ...
        train_samples.append(
            InputExample(
                texts=[row["question2"], row["question1"]],
                label=int(row["is_duplicate"]),
            )
        )
...
dev_samples = []
with open(
    os.path.join(dataset_path, "classification", "dev_pairs.tsv"), "r", encoding="utf8"
) as fIn:
    reader = csv.DictReader(fIn, delimiter="\t", quoting=csv.QUOTE_NONE)
    for row in reader:
        dev_samples.append(
...
train_batch_size = 16
num_epochs = 4
model_save_path = "output/training_quora-" + datetime.now().strftime(
    "%Y-%m-%d_%H-%M-%S"
)
