...
logging.info(
    "Step 2.1: Generate STSbenchmark (silver dataset) using top-{} bm25 combinations".format(
        top_k
    )
)
...
sent2idx = {
    sentence: idx for idx, sentence in enumerate(unique_sentences)
...
for sent, idx in sent2idx.items():
    res = es.search(
        index=index_name, body={"query": {"match": {"sent": sent}}}, size=top_k
    )
    progress.update(1)
...
logging.info(
    "Number of silver pairs generated for STSbenchmark: {}".format(len(silver_data))
)
logging.info(
    "Step 2.2: Label STSbenchmark (silver dataset) with cross-encoder: {}".format(
        model_name
    )
)
...
logging.info(
    "Step 3: Train bi-encoder: {} with STSbenchmark (gold + silver dataset)".format(
        model_name
    )
)
...
silver_samples = list(
    ...
    for data, score in zip(silver_data, silver_scores)
)


train_dataloader = DataLoader(
    gold_samples + silver_samples, shuffle=True, batch_size=batch_size
)
