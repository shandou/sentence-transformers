...
output_path = "output/model-distillation-" + datetime.now().strftime(
    "%Y-%m-%d_%H-%M-%S"
)
...
if use_layer_reduction:
    ...
    logging.info(
        "Remove layers from student. Only keep these layers: {}".format(layers_to_keep)
    )
...
if (
    student_model.get_sentence_embedding_dimension()
    < teacher_model.get_sentence_embedding_dimension()
):
    logging.info(
        "Student model has fewer dimensions than the teacher. Compute PCA for down projection"
    )
