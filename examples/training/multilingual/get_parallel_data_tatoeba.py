...
num_dev_sentences = (
    1000  # Number of sentences that are used to create a development set
)
...
translations = {
    src_lang: {trg_lang: {} for trg_lang in target_languages}
...
for src_lang in source_languages:
    for trg_lang in target_languages:
        ...
        print(
            "{}-{} has {} sentences".format(src_lang, trg_lang, len(source_sentences))
        )
        if len(dev_sentences) > 0:
            with gzip.open(
                os.path.join(
                    output_folder, "Tatoeba-{}-{}-dev.tsv.gz".format(src_lang, trg_lang)
                ),
                "wt",
                encoding="utf8",
            ) as fOut:
                for sent in dev_sentences:
                    fOut.write(
                        "\t".join([sent] + translations[src_lang][trg_lang][sent])
                    )
                    fOut.write("\n")
        ...
        if len(train_sentences) > 0:
            with gzip.open(
                os.path.join(
                    output_folder,
                    "Tatoeba-{}-{}-train.tsv.gz".format(src_lang, trg_lang),
                ),
                "wt",
                encoding="utf8",
            ) as fOut:
