...
target_languages = set(
    ["de", "es", "it", "fr", "ar", "tr"]
)  # New languages we want to extend to


num_dev_sentences = 1000  # Number of sentences we want to use for development
threshold = (
    1.075  # Only use sentences with a LASER similarity score above the threshold
)
...
for source_lang in source_languages:
    for target_lang in target_languages:
        filename_train = os.path.join(
            ...
            "WikiMatrix-{}-{}-train.tsv.gz".format(source_lang, target_lang),
        )
        filename_dev = os.path.join(
            parallel_sentences_folder,
            "WikiMatrix-{}-{}-dev.tsv.gz".format(source_lang, target_lang),
        )
        ...
        if not os.path.exists(filename_train) and not os.path.exists(filename_dev):
            ...
            if not os.path.exists(wikimatrix_filepath):
                ...
                try:
                    ...
                    print(
                        "Was not able to download", download_url + wikimatrix_filename
                    )
                    continue
            ...
            with gzip.open(wikimatrix_filepath, "rt", encoding="utf8") as fIn:
                for line in fIn:
                    ...
                    if (
                        langs_ordered.index(source_lang) == 1
                    ):  # Swap, so that src lang is sent1
