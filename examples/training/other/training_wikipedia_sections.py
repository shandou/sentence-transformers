...
if not os.path.exists(dataset_path):
    ...
    util.http_get(
        "https://sbert.net/datasets/wikipedia-sections-triplets.zip", filepath
    )
...
train_batch_size = 16
output_path = (
    "output/training-wikipedia-sections-"
    + model_name
    + "-"
...
with open(os.path.join(dataset_path, "train.csv"), encoding="utf-8") as fIn:
    ...
    for row in reader:
        train_examples.append(
            InputExample(
                texts=[row["Sentence1"], row["Sentence2"], row["Sentence3"]], label=0
            )
        )
...
with open(os.path.join(dataset_path, "validation.csv"), encoding="utf-8") as fIn:
    ...
    for row in reader:
        dev_examples.append(
            InputExample(texts=[row["Sentence1"], row["Sentence2"], row["Sentence3"]])
        )
