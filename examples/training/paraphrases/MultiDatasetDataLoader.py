...
class MultiDatasetDataLoader:
    def __init__(
        self, datasets, batch_size_pairs, batch_size_triplets=None, dataset_size_temp=-1
    ):
        self.allow_swap = True
        self.batch_size_pairs = batch_size_pairs
        self.batch_size_triplets = (
            batch_size_pairs if batch_size_triplets is None else batch_size_triplets
        )
        ...
        if dataset_size_temp > 0:  # Scale probability with dataset size
            for dataset in datasets:
                ...
                weights.append(
                    max(1, int(math.pow(prob, 1 / dataset_size_temp) * 1000))
                )
        else:  # Equal weighting of all datasets
            weights = [100] * len(datasets)

        logging.info(
            "Dataset lenghts and weights: {}".format(
                list(zip(self.dataset_lengths, weights))
            )
        )
    ...
    def __iter__(self):
        for _ in range(int(self.__len__())):
            ...
            batch_size = (
                self.batch_size_pairs
                if len(dataset["elements"][0].texts) == 2
                else self.batch_size_triplets
            )
            ...
            while len(batch) < batch_size:
                ...
                if valid_example:
                    if self.allow_swap and random.random() > 0.5:
                        example.texts[0], example.texts[1] = (
                            example.texts[1],
                            example.texts[0],
                        )
