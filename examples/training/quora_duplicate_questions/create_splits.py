...
if not os.path.exists(source_file):
    ...
    util.http_get(
        "http://qim.fs.quoracdn.net/quora_duplicate_questions.tsv", source_file
    )
...
with open(source_file, encoding="utf8") as fIn:
    ...
    for row in reader:
        ...
        id2 = row["qid2"]
        question1 = (
            row["question1"].replace("\r", "").replace("\n", " ").replace("\t", " ")
        )
        question2 = (
            row["question2"].replace("\r", "").replace("\n", " ").replace("\t", " ")
        )
...
for id, sent in sentences.items():
    ...
    sent_norm = sent_norm.replace("how do you", "how do i").replace(
        "how do we", "how do i"
    )
    sent_norm = (
...
with open(
    "quora-IR-dataset/graph/duplicates-graph-pairwise.tsv", "w", encoding="utf8"
) as fOut:
...
with open(
    "quora-IR-dataset/graph/duplicates-graph-list.tsv", "w", encoding="utf8"
) as fOut:
    fOut.write("qid1\tqid2\n")
...
with open(
    "quora-IR-dataset/graph/duplicates-graph-connected-nodes.tsv", "w", encoding="utf8"
) as fOut:
    written_qids = set()
    fOut.write("qids\n")
    for a in sorted(duplicates.keys(), key=lambda x: int(x)):
...
def write_qids(name, ids_list):
    with open(
        "quora-IR-dataset/graph/" + name + "-questions.tsv", "w", encoding="utf8"
    ) as fOut:
        fOut.write("qid\n")
...
def write_mining_files(name, ids, dups):
    with open(
        "quora-IR-dataset/duplicate-mining/" + name + "_corpus.tsv",
        "w",
        encoding="utf8",
    ) as fOut:
        fOut.write("qid\tquestion\n")
    ...
    with open(
        "quora-IR-dataset/duplicate-mining/" + name + "_duplicates.tsv",
        "w",
        encoding="utf8",
    ) as fOut:
        fOut.write("qid1\tqid2\n")
...
with open(
    "quora-IR-dataset/classification/train_pairs.tsv", "w", encoding="utf8"
) as fOutTrain, open(
    "quora-IR-dataset/classification/dev_pairs.tsv", "w", encoding="utf8"
) as fOutDev, open(
    "quora-IR-dataset/classification/test_pairs.tsv", "w", encoding="utf8"
) as fOutTest:
    fOutTrain.write(
        "\t".join(["qid1", "qid2", "question1", "question2", "is_duplicate"]) + "\n"
    )
    fOutDev.write(
        "\t".join(["qid1", "qid2", "question1", "question2", "is_duplicate"]) + "\n"
    )
...
with open(
    "quora-IR-dataset/information-retrieval/corpus.tsv", "w", encoding="utf8"
) as fOut:
    fOut.write("qid\tquestion\n")
...
with open(
    "quora-IR-dataset/information-retrieval/dev-queries.tsv", "w", encoding="utf8"
) as fOut:
...
with open(
    "quora-IR-dataset/information-retrieval/test-queries.tsv", "w", encoding="utf8"
) as fOut:
    fOut.write("qid\tquestion\tduplicate_qids\n")
